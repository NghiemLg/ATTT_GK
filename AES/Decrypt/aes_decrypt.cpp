...
int main() {
    ...
    if (cipher_bin.size() % 16 != 0) return 1; // Error

    // Decrypt
    ExpandedKey expanded_key = aes_key_expansion(key);
    std::vector<uint8_t> plaintext;
    for (size_t i = 0; i < cipher_bin.size(); i += 16) {
        Block block;
        std::copy(cipher_bin.begin() + i, cipher_bin.begin() + i + 16, block.begin());
        Block decrypted = aes_decrypt_block(block, expanded_key);
